...
class UDPServer:
    ...
    def getFile (self, filename):
        try:
            ...
            self.file = file.read()
            self.server.sendto(Package("Arquivo encontrado".encode("utf-8"), 0).getPackage(), self.currentClientAddress)
            return True
        except FileNotFoundError:
            self.server.sendto(Package("Arquivo não encontrado".encode("utf-8"), 0).getPackage(), self.currentClientAddress)
            return False
    ...
    def sendFile(self):
        ...
        while i < len(segments) + 1:
            package = Package(segments[i - 1], i).getPackage()
...
def main():
    server = UDPServer(5555,"127.0.0.1")
